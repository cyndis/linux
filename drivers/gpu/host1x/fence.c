...
int host1x_sync_fence_count_waits(struct sync_fence *fence)
{
	struct sync_timeline *tl;
	int i, count = 0;

	for (i = 0; i < fence->num_fences; ++i) {
		tl = sync_pt_parent((struct sync_pt *)fence->cbs[i].sync_pt);
		if (tl->ops == &host1x_timeline_ops)
			count += 1;
	}
...
void host1x_sync_fence_unpack_waits(struct sync_fence *fence,
				    struct host1x_waitchk *waitchk)
{
	...
	for (i = 0; i < fence->num_fences; ++i) {
		...
		if (host1x_sync_pt_extract(spt, &syncpt, &threshold)) {
			waitchk->bo = NULL;
			waitchk->offset = 0;
			waitchk->syncpt_id = syncpt->id;
			waitchk->thresh = threshold;
			++waitchk;
		}
	}
}
